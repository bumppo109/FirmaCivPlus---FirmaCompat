    ...
    private static void onRegisterReloadListeners(final RegisterClientReloadListenersEvent evt)
    {
        ...
        for(final var woodEntry : ModWatercraftMaterial._ALL_WATERCRAFT_MATERIALS)
        {
            ...
            evt.registerReloadListener(new PaintedTextureGenerator(
                    new ResourceLocation(FirmaCivFirmaCompat.MOD_ID, "entity/watercraft/rowboat/" + woodEntry.getSerializedName()), rowboatPaint
            ));

            evt.registerReloadListener(new PaintedTextureGenerator(
                    new ResourceLocation(FirmaCivFirmaCompat.MOD_ID, "entity/watercraft/sloop/" + woodEntry.getSerializedName()), sloopPaint
            ));
        }
    }
}
